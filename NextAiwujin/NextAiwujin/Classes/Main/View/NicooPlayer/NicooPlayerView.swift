...
open class NicooPlayerView: UIView {
    ...
    /// 嵌入式播放控制View
    lazy var playControllViewEmbed: NicooPlayerControlView = {
    ...
    public var isLive = false {
        didSet{
            if isLive == true {
                self.playControllViewEmbed.timeSlider.isHidden = true
                self.playControllViewEmbed.loadedProgressView.isHidden = true
                self.playControllViewEmbed.positionTimeLab.isHidden = true
                self.playControllViewEmbed.durationTimeLab.isHidden = true
            }
    ...
    public init(frame: CGRect, bothSidesTimelable: Bool? = false, isLive:Bool = false) {
...
//                make.bottom.equalTo(playControllViewEmbed.videoNameLable)
                make.centerY.equalTo(playControllViewEmbed.videoNameLable)
...
//                print("非全屏状态，停止播放，移除播放视图")
                //播放器全屏状态下发送退出vc的通知
                NotificationCenter.default.post(name: PopVCWhenPlayerNotFullScreenNotificationName, object: nil)
            }
        }
        // MARK: - 全屏
    ...
    @objc func orientChange(_ sender: Notification) {
        ...
        if  orirntation == UIInterfaceOrientation.landscapeLeft || orirntation == UIInterfaceOrientation.landscapeRight  {
            isFullScreen = true
            self.removeFromSuperview()
//            self.videoLayerGravity = .resize
            self.translatesAutoresizingMaskIntoConstraints = false
            self.fatherView?.superview?.addSubview(self)
            
            UIView.animate(withDuration: 0.2, delay: 0, options: UIView.AnimationOptions.transitionCurlUp, animations: {
                self.snp.makeConstraints({ (make) in
                    make.edges.equalTo((self.fatherView?.superview)!)

                })
//                self.topAnchor.constraint(equalTo: UIApplication.shared.keyWindow!.topAnchor).isActive = true
//                self.leftAnchor.constraint(equalTo: UIApplication.shared.keyWindow!.leftAnchor).isActive = true
//                self.rightAnchor.constraint(equalTo: UIApplication.shared.keyWindow!.rightAnchor).isActive = true
//                self.bottomAnchor.constraint(equalTo: UIApplication.shared.keyWindow!.bottomAnchor).isActive = true
                
//                UIDevice.current.isiPhoneXSeriesDevices() //详细判断ios机型
...
//            NLog("loadedTimeRanges")
            ...
            if !isDragging && playerStatu != PlayerStatus.Pause {
                //MARK: - 此处原本为show，改为hide后直播流加载t菊花不会一直显示了
                hideLoadingHud()
            }
        }
    }
    ...
    private func updateTimeSliderValue(avItem: AVPlayerItem) {
        ...
        if avItem.asset.duration.value > 0 && avItem.currentTime().value > 0 {
            ...
            //视频信息加载完成后发送通知，通知中包含视频时长信息
            NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "getNowPlayPositionTimeAndVideoDuration"), object: stringDuration)
