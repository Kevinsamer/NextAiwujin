...
//        let indexPath = MyCollectionView.indexPath(for: cell)!
        ...
        let alert = UIAlertController(title: "提示", message: "是否取消收藏", preferredStyle: UIAlertController.Style.alert)
        let OKAction = UIAlertAction(title: "是", style: UIAlertAction.Style.default) { [unowned self] (OK) in
            self.collNum -= 1
            self.MyCollectionView.reloadData({[unowned self] in
                YTools.showMyToast(rootView: self.view, message: "取消成功",position: .center)
            })
        }
        let CancelAction = UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel) { (Cancel) in
