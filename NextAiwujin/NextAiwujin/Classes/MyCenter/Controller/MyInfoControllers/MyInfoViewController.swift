...
class MyInfoViewController: FormViewController {
    ...
    lazy var loginOutAlert: UIAlertController = {
        let alert = UIAlertController(style: UIAlertController.Style.alert, source: nil, title: nil, message: "是否退出登录？", tintColor: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))
        let cancelAction = UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel, handler: { (action) in
