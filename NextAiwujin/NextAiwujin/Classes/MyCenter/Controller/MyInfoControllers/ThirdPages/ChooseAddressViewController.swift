...
class ChooseAddressViewController: BaseViewController {
    ...
    lazy var delAlert: UIAlertController = {
        let alert = UIAlertController(style: UIAlertController.Style.alert)
        alert.title = "是否删除"
        let okAction = UIAlertAction(title: "确认", style: UIAlertAction.Style.default, handler: {[unowned self] (action) in
            //确认删除
    ...
    lazy var noAddressButton: UIButton = {
        let imageB = UIButton(type: UIButton.ButtonType.custom)
    ...
    lazy var newAddressButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(addNewAddress), for: UIControl.Event.touchUpInside)
        return button
    }()
