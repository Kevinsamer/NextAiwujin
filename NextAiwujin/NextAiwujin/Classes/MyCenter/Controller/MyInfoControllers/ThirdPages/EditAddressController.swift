...
class EditAddressController: FormViewController {
    var addressID:Int?
    var name:String?
    var phoneNum:String?
    var address:(Area,Area,Area)?
    var currentAddress:(String,String,String)?//已选中的地址
...
//MARK: - 设置ui
extension EditAddressController {
    private func setUI(){
...
//        navBarTitleColor = .white
    ...
    private func setupBodyContent(){
        form +++ Section()
            ...
            <<< LabelRow("address"){
                $0.title = "所在地区:"
                if let cAddress = currentAddress {
                    $0.value = "\(cAddress.0) \(cAddress.1) \(cAddress.2)"
                }else{
                    $0.value = "点击选择所在地区 ＞"
                }
                ...
                $0.validationOptions = .validatesOnChange
                    ...
                    right.contentMode = UIView.ContentMode.center
            ...
            <<< ButtonRow("saveButton"){
                $0.title = "保存"
                    ...
                    if errors.count != 0 {
                        ...
                        self.myCenterViewModel.requestEditAddress(id:id!, acceptName: accept_name, province: province!, city: city ?? 0, area: area ?? 0, address: address, zip: zip, telphone: telphone, mobile: mobile, finishCallback: { msg in
                            if msg == ""{
                                //添加成功
