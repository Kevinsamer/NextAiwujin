...
//    }()
    ...
    lazy var footer: DefaultRefreshFooter = {
        ...
        footer.setText("——— 我是有底线的 ———", mode: .tapToRefresh)
    ...
    lazy var ordersTableView: UITableView = {
        let tableView = UITableView(frame: CGRect.init(x: 0, y: finalStatusBarH + finalNavigationBarH, width: finalScreenW, height: finalContentViewNoTabbarH - 44), style: UITableView.Style.plain)//44为buttonBarView的高度
...
//            }
            ...
            }else if (orderList[indexPath.row].distribution_status == 1 || orderList[indexPath.row].distribution_status == 0 || orderList[indexPath.row].distribution_status == 2) && orderList[indexPath.row].status == 2 {
                if (orderList[indexPath.row].distribution_status == 1 || orderList[indexPath.row].distribution_status == 2) {
                    cell.orderStateLabel.text = "待收货"
                }else if (orderList[indexPath.row].distribution_status == 0) {
