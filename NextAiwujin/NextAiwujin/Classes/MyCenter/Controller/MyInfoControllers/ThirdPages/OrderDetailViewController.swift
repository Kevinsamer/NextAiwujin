    ...
    var orderModel:OrderModel?{
        didSet{
            ...
            if let order = orderModel {
                if order.status == 1 && order.pay_status == 0 {
                ...
                }else if (order.distribution_status == 1 || order.distribution_status == 0 || order.distribution_status == 2) && order.status == 2{
                    if (order.distribution_status == 1 || order.distribution_status == 2){
                        orderState = "待收货"
                    ...
                    }else if order.distribution_status == 0 {
                        orderState = "待发货"
                        cancelOrderButton.removeFromSuperview()
                        nextStepButton.setTitleForAllStates("请等待商家发货")
...
//                    bottomView.removeFromSuperview()
    ...
    ///确认收货alert
    lazy var confirmAlert: UIAlertController = {
        let alert = UIAlertController(title: "是否确认收货？", message: nil, preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel, handler: nil))
        return alert
    }()
    
    lazy var confirmOKAction: UIAlertAction = {
        let action = UIAlertAction(title: "确认", style:UIAlertAction.Style.default, handler: { (action) in
            ...
            //NotificationCenter.default.post(name: ConfirmOrderNotificationName, object: self, userInfo: nil)
        })
        return action
    }()
    
    lazy var cancelAlert: UIAlertController = {
        let alert = UIAlertController(title: "订单取消后无法恢复，确认取消？", message: nil, preferredStyle:UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel, handler: nil))
        return alert
    }()
    ...
    lazy var cancelOrderButton: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cancelOrder), for: UIControl.Event.touchUpInside)
        return btn
    }()
    ...
    lazy var orderGoodsTabelView: UITableView = {
        let table = UITableView(frame: CGRect.init(x: 0, y: finalStatusBarH + finalNavigationBarH, width: finalScreenW, height: finalContentViewNoTabbarH - bottomViewH), style:UITableView.Style.grouped)
        table.delegate = self
...
//        initData()
        ...
        setContentView()
        //接收操作订单通知
//        self.addNotificationObserver(name: ConfirmOrderNotificationName, selector: #selector(confirmNoti))
//        self.addNotificationObserver(name: CancelOrderNotificationName, selector: #selector(cancelNoti))
    ...
    private func setContentView(){
        ...
        self.confirmAlert.addAction(self.confirmOKAction)
