...
class FindPasswordTabViewController: UIViewController, IndicatorInfoProvider {
    ...
    lazy var sendAuthCode: UIButton = {
        let authCode = UIButton(type: UIButton.ButtonType.custom)
        ...
        authCode.addTarget(self, action: #selector(sendAuthCodeClicked), for: UIControl.Event.touchUpInside)
        return authCode
    }()
    ...
    lazy var findButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(findButtonClicked), for: UIControl.Event.touchUpInside)
        return button
    }()
