...
//            guard let resultDict = result as? [String: NSObject] else {return}
    ...
    func requestUserAddress(id:Int, finishCallback:@escaping ()->()){
        NetworkTool.requestData(type: .POST, urlString: GETADDRESS_URL, parameters: ["id":"\(id)" as NSString]) { (result) in
    ...
    func requestAddAddress(acceptName:String, province:Int, city:Int, area:Int, address:String, zip:String, telphone:String, mobile:String, finishCallback:@escaping (_ msg:String)->()){
        self.resultMsg = ""
        NetworkTool.requestData(type: .POST, urlString: ADDADDRESS_URL, parameters: ["accept_name":"\(acceptName)" as NSString, "province":"\(province)" as NSString, "city":"\(city)" as NSString, "area":"\(area)" as NSString, "address":"\(address)" as NSString, "zip":"\(zip)" as NSString, "telphone":"\(telphone)" as NSString, "mobile":"\(mobile)" as NSString]) { (result) in
    ...
    func requestIsDefaultAddress(user_id id:Int, finishCallback:@escaping(_ address:MyAddressModel)->()){
        NetworkTool.requestData(type: .POST, urlString: ISDEFAULTADDRESS_URL, parameters: ["user_id":"\(id)" as NSString]) { (result) in
            let resultJSON = JSON(result)
    ...
    func requestOrderList(user_id id:Int, page:Int, query_what query:String, finishCallback:@escaping (_ orderLists:[OrderModel])->()){
        NetworkTool.requestData(type: .POST, urlString: MYORDERLIST_URL, parameters: ["user_id":"\(id)" as NSString, "page":"\(page)" as NSString, "query_what":"\(query)" as NSString]) { (result) in
    ...
    func requestOrderGoodsList(order_id id:Int, finishCallback:@escaping (_ goodsLists:[OrderGoodsListModel])->()){
        NetworkTool.requestData(type: .POST, urlString: ORDERGOODS_URL, parameters: ["order_id":"\(id)" as NSString]) { (result) in
            let resultData = JSON(result)
            //print(resultData)
            let jsonData = resultData["result"].arrayValue
    ...
    func requestOrderDetail(order_id id:Int, isPhone:Bool = true, finishCallback:@escaping (_ order:OrderModel)->()){
        NetworkTool.requestData(type: MethodType.POST, urlString: ORDERDETAIL_URL, parameters: ["id":"\(id)" as NSString, "is_phone":"\(isPhone)" as NSString]) { (result) in
    ...
    func requestPasswordEdit(fpassword:String, password:String, repassword:String, finishaCallback:@escaping (_ editResult:String)->()){
        NetworkTool.requestData(type: .POST, urlString: PASSWORDEDIT_URL, parameters: ["fpassword":"\(fpassword)" as NSString, "password":"\(password)" as NSString, "repassword":"\(repassword)" as NSString]) { (result) in
    ...
    func requestInfoEditAct(true_name name:String, sex:String, birthday:String, mobile:String, email:String, zip:String, teltphone:String, qq:String, finishCallback:@escaping (_ resultCode:Int)->()){
        NetworkTool.requestData(type: .POST, urlString: INFOEDITACT_URL, parameters: ["true_name":"\(name)" as NSString, "sex":"\(sex)" as NSString, "birthday":"\(birthday)" as NSString, "mobile":"\(mobile)" as NSString, "email":"\(email)" as NSString, "zip":"\(zip)" as NSString, "telephone":"\(teltphone)" as NSString, "qq":"\(qq)" as NSString]) { (result) in
