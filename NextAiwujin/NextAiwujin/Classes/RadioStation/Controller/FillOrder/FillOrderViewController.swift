...
class FillOrderViewController: BaseViewController {
    ...
    private var selectedAddress:MyAddressModel?{
        willSet{
            ...
            DispatchQueue.main.async {
                self.initOrderDelivery(selectAddress: newValue ?? MyAddressModel(jsonData: ""))
            }
    ...
    lazy var payModeAlert: MyAlertController = {
        let alert = MyAlertController(title: "支付方式", message: "请选择付款方式", preferredStyle: UIAlertController.Style.actionSheet)
        return alert
    }()
    ...
    lazy var goodsInfoTableView: UITableView = {
        let table = UITableView(frame: CGRect.init(x: 0, y: 0, width: finalScreenW - 20, height: CGFloat(goodsList?.count ?? 1) * goodsCellH), style: UITableView.Style.plain)
    ...
    lazy var sendModeView: UIControl = {
        ...
        view.addTarget(self, action: #selector(showSendModeAlert), for: UIControl.Event.touchUpInside)
        return view
    }()
    
    lazy var sendModeAlert: MyAlertController = {
        let alert = MyAlertController(title: "配送方式", message: "请选择配送方式", preferredStyle: UIAlertController.Style.actionSheet)
        return alert
    }()
    ...
    lazy var invoiceSwitch: UISwitch = {
        ...
        switchButton.addTarget(self, action: #selector(invoiceSwitchValueChanged), for: UIControl.Event.valueChanged)
        return switchButton
    }()
    ...
    lazy var vouchersView: UIControl = {
        ...
        view.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
        view.addTarget(self, action: #selector(showVouchersAlert), for: UIControl.Event.touchUpInside)
        return view
    }()
    ...
    lazy var collView: UICollectionView = {
        ...
        coll.keyboardDismissMode = UIScrollView.KeyboardDismissMode.onDrag
    ...
    lazy var onlineBtn = UIAlertAction(title: "支付宝支付", style: UIAlertAction.Style.default, handler: { (action) in
    ...
    //    })
    let cancleBtn = UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel, handler: nil)
        ...
        self.orderViewModel.requestProvinceIDByName(provinceName: selectAddress.province) { (provinceId) in
            ...
            self.orderViewModel.requestOrderDelivery(goodsList: self.goodsList!, province: provinceId) {[unowned self] (kuaidi,ziti) in
                DispatchQueue.main.async(execute: {
                    self.sendModeAlert = MyAlertController(title: "配送方式", message: "请选择配送方式", preferredStyle: UIAlertController.Style.actionSheet)
...
extension FillOrderViewController:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.goodsImageView.kf.setImage(with: URL.init(string: BASE_URL + "\(goodsList![indexPath.row].img)"), placeholder: UIImage.init(named: "loading"))
