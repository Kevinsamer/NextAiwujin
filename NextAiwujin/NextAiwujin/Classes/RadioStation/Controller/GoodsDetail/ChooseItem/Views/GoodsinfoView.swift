...
class GoodsinfoView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        goodsImage.contentMode = UIView.ContentMode.scaleAspectFill
        ...
        //关闭按钮
        closeButton = UIButton.init(type: UIButton.ButtonType.custom)
        closeButton.setImage(UIImage.init(named: "guanbi"), for: UIControl.State.normal)
    ...
    func initData(model:GoodsModel){
        _model = model
        goodsImage.kf.setImage(with: URL.init(string: BASE_URL + model.imageId), placeholder: UIImage.init(named: "loading"))
        ...
        attritu.addAttributes([NSAttributedString.Key(rawValue: NSAttributedString.Key.foregroundColor.rawValue): UIColor.lightGray,NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12),NSAttributedString.Key(rawValue: NSAttributedString.Key.baselineOffset.rawValue):0,NSAttributedString.Key(rawValue: NSAttributedString.Key.strikethroughStyle.rawValue):1], range: range)
    ...
    func resetData(sizeModel:SizeAttributeModel) {
        ...
        if sizeModel.imageId.isEmpty
        {
            ...
            goodsImage.kf.setImage(with: URL.init(string: BASE_URL + _model.imageId), placeholder: UIImage.init(named: "loading"))
        }else{
            //goodsImage.image = UIImage(named: _model.imageId)
            goodsImage.kf.setImage(with: URL.init(string: BASE_URL + sizeModel.imageId), placeholder: UIImage.init(named: "loading"))
        }
