...
class TypeView: UIView {
    ...
    func initWith(arr:NSArray,typeName:String,index:Int)
    {
        ...
        for i:Int in 0..<arr.count
        {
            ...
            btn.setTitle(text, for: UIControl.State.selected)
            btn.setTitleColor(UIColor.black, for: UIControl.State.normal)
            btn.setTitleColor(UIColor.white, for: UIControl.State.selected)
            btn.backgroundColor = UIColor(named: "line_gray")!
            
            let dic = NSDictionary(object: UIFont.systemFont(ofSize: 13), forKey: NSAttributedString.Key.font as NSCopying)
            
            let size: CGSize = text.size(withAttributes: (dic as! [NSAttributedString.Key : Any]))
            ...
            self.addSubview(btn)
            btn.tag = 100+i
            btn.addTarget(self, action: #selector(self.touchbtn), for: UIControl.Event.touchUpInside)
