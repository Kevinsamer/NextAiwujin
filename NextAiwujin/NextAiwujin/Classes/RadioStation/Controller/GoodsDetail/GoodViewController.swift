...
class GoodViewController: GoodDetailBaseViewController {
    ...
    lazy var goodsChosenInfoView: UIControl = {
        ...
        view.backgroundColor = .white
        view.addTarget(self, action: #selector(popChooseView), for: UIControl.Event.touchUpInside)
        return view
    }()
    ...
    lazy var tableView: UITableView = {
        let table = UITableView(frame: CGRect.init(x: 0, y: finalStatusBarH, width: finalScreenW, height: self.usableViewHeight!), style: UITableView.Style.plain)
        table.delegate = self
...
extension GoodViewController {
    func goodPriceString(_ newPrice: String, _ oldPrice: String) -> NSMutableAttributedString {
        ...
        attributedString.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.red , NSAttributedString.Key.font : UIFont.systemFont(ofSize: 20).bold], range: rang1)
        attributedString.addAttributes([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 16) , NSAttributedString.Key.baselineOffset : 0 , NSAttributedString.Key.strikethroughStyle : 2 , NSAttributedString.Key.foregroundColor : UIColor.lightGray], range: rang2)
    ...
    func goodChosenString(_ chosenInfo: String) -> NSMutableAttributedString {
        ...
        attributedString.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.lightGray , NSAttributedString.Key.font : UIFont.systemFont(ofSize: 12)], range: rang1)
        attributedString.addAttributes([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 16).bold , NSAttributedString.Key.foregroundColor : UIColor.black], range: rang2)
        return attributedString
    }
}
