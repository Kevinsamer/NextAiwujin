...
class PayViewController: BaseViewController {
    ...
    private var zfbString:String?{
        didSet{
            if let zfbStr = zfbString{
                //print(zfbStr)
                AlipaySDK.defaultService().payOrder(zfbStr, fromScheme: "aiwujinmall") { (resultDic) in
    ...
    lazy var payingAlert: UIAlertController = {
        let alert = UIAlertController(title: "支付处理中,请稍等", message: nil, preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction.init(title: "返回我的订单查看支付结果", style: UIAlertAction.Style.cancel, handler: { (action) in
    ...
    lazy var failedAlert: UIAlertController = {
        let alert = UIAlertController(title: "支付失败", message: nil, preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction.init(title: "重新支付", style: UIAlertAction.Style.cancel, handler: nil))
        return alert
    }()
    
    lazy var successAlert: UIAlertController = {
        let alert = UIAlertController(title: "支付成功", message: nil, preferredStyle: UIAlertController.Style.alert)
    ...
    lazy var contentTabelView: UITableView = {
        let table = UITableView(frame: CGRect.init(x: 0, y: finalStatusBarH + finalNavigationBarH, width: finalScreenW, height: finalScreenH), style: UITableView.Style.grouped)
    ...
    lazy var leftBarButtonItem: UIBarButtonItem = {
        let item = UIBarButtonItem(image: #imageLiteral(resourceName: "btnBack"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(cancelPay))
        return item
    }()
    
    lazy var goOnPayAction: UIAlertAction = {
        let action = UIAlertAction(title: "继续支付", style: UIAlertAction.Style.cancel, handler: { (action) in
    ...
    lazy var cancelPayAction: UIAlertAction = {
        let action = UIAlertAction(title: "确认离开", style: UIAlertAction.Style.default, handler: { (action) in
            let vc = MyOrdersViewController()
            vc.isFromPayVC = true
            self.navigationController?.show(vc, sender: self)
        })
    ...
    lazy var cancelAlert: UIAlertController = {
        let alert = UIAlertController(title: "确认要取消支付？", message: "", preferredStyle: UIAlertController.Style.alert)
...
extension PayViewController {
    @objc private func doPay(){
        //print(AliSDKManager.signInfo(bizContent: biz))
        //调起支付宝
//        print("DOPAY")
        orderViewModel.requestZFBString(body: biz.body, subject: biz.subject, out_order_no: biz.out_trade_no, total_amount: biz.total_amount) { (zfbString) in
            self.zfbString = zfbString
//            print(zfbString)
        }
            ...
            case "9000":
                ...
                for id in self.order_ids{
                    ...
                    mQueue.async(group: group, qos: .default, flags: [], execute: {
                        self.orderViewModel.requestUpdateOrder(order_id: id, status: 2, pay_status: 1, finishCallback: { (updateResult) in
                            //TODO:该接口发现一个问题，支付成功后没有同步修改数据库支付状态字段pay_status
                            self.totalPayResult = updateResult
                            group.leave()
                        })
                    })
