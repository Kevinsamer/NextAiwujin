//
//  NextShopCartViewController.swift
//  NextAiwujin
//  二级购物车页面
...
class NextShopCartViewController: BaseViewController {
    ...
    lazy var refreshIndicator: UIActivityIndicatorView = {
        let indi = UIActivityIndicatorView(style: UIActivityIndicatorView.Style.gray)
...
//        initData()
    ...
    func setupBottomView() {
        ...
        seletAllButton.setTitle("  全选", for: UIControl.State())
        seletAllButton.setImage(UIImage(named: "cart_unSelect_btn"), for: UIControl.State())
        seletAllButton.setImage(UIImage(named: "cart_selected_btn"), for: UIControl.State.selected)
        seletAllButton.setTitleColor(UIColor.black, for: UIControl.State())
        seletAllButton.addTarget(self, action: #selector(selectAllButtonClick), for: UIControl.Event.touchUpInside)
        ...
        commitButton.setTitle("去结算", for: UIControl.State())
        commitButton.addTarget(self, action: #selector(comitButtonClick), for: UIControl.Event.touchUpInside)
    ...
    func numString(_ num: String) -> NSMutableAttributedString{
        ...
        attributedString.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.black , NSAttributedString.Key.font : UIFont.systemFont(ofSize: 16)], range: rang1)
        attributedString.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.black , NSAttributedString.Key.font : UIFont.systemFont(ofSize: 16)], range: rang2)
    ...
    private func setupTableView(){
        cartTableView = UITableView(frame: CGRect.init(x: 0, y: finalStatusBarH + finalNavigationBarH, width: finalScreenW, height: finalScreenH - finalStatusBarH - finalNavigationBarH - (UIDevice.current.isX() ? IphonexHomeIndicatorH + finalTabBarH : finalTabBarH)),style: UITableView.Style.plain)
    ...
    private func setupNavigationBar(){
        ...
        rightBtn.addTarget(self, action: #selector(editButtonClicked), for: UIControl.Event.touchUpInside)
...
extension NextShopCartViewController{
    ...
    @objc func comitButtonClick() {
        if editButtonState == 1 {
            //编辑状态
            if delArray.count == 0 {
                let alert = UIAlertController(title: "提示", message: "请选择要删除的商品", preferredStyle: UIAlertController.Style.alert)
                let ok = UIAlertAction(title: "确定", style: UIAlertAction.Style.cancel, handler: { (cancle) in
                    
                })
                alert.addAction(ok)
                self.present(alert, animated: true, completion: nil)
            }else{
                let alert = UIAlertController(title: "提示",message: "删除后无法恢复,是否继续删除?",preferredStyle: UIAlertController.Style.alert)
                
                let ok = UIAlertAction(title: "确定",style: UIAlertAction.Style.default,handler:{ [unowned self] okAction in
                ...
                let cancel = UIAlertAction(title: "取消",style: UIAlertAction.Style.cancel,handler: {cancelAction in
...
extension NextShopCartViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            cell = LZCartTableViewCell(style: UITableViewCell.CellStyle.default,reuseIdentifier: "cartCellID")
        }
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let actionColl = UITableViewRowAction(style: UITableViewRowAction.Style.default, title: "添加收藏") { (action, index) in
            print("收藏\(indexPath.row)")
            
        }
        actionColl.backgroundColor = UIColor(named: "global_orange")
        let actionDel = UITableViewRowAction(style: UITableViewRowAction.Style.default, title: "删除") { (action, index) in
            print("删除\(indexPath.row)")
            let alert = UIAlertController(title: "提示",message: "删除后无法恢复,是否继续删除?",preferredStyle: UIAlertController.Style.alert)
            
            let ok = UIAlertAction(title: "确定",style: UIAlertAction.Style.default,handler:{okAction in
