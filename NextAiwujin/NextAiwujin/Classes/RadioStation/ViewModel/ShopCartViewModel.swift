...
extension ShopCartViewModel{
    ...
    func requestRemoveCart(goods_id id:Int, type:JoinCartType,finishedCallback:@escaping (_ remove:RemoveCartModel)->()){
        NetworkTool.requestData(type: MethodType.POST, urlString: REMOVECART_URL, parameters: ["goods_id":"\(id)" as NSString, "type":"\(type)" as NSString]) { (result) in
    ...
    func requestCart(finishedCallback: @escaping ()->()){
        NetworkTool.requestData(type: .GET, urlString: CARTINFO_URL) { (result) in
